...
function uploadSuccess(file, serverData) {
	try {
		...
		if (serverData.substring(0, 7) === "FILEID:") {
			...
			progress.setStatus("Upload Completo.");
...
function uploadComplete(file) {
	try {
		...
		if (this.getStats().files_queued > 0) {
			...
			progress.setStatus("Todas as imagens foram recebidas.");
			progress.toggleCancel(false);
		}
	} catch (ex) {
		this.debug(ex);
...
function addImage(src, serverData) {
	...
	newDiv.id 				= "div"+serverData;
	newDiv.style.width 		= "150px";
	newDiv.style.styleFloat = "left";
	newDiv.style.cssFloat 	= "left";
	newDiv.style.height 	= "79px";
	newDiv.innerHTML		= "<a href='#' onclick=\"removerNormal('"+serverData+"')\">Remover</a>";
