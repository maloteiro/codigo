...
%><w:initPage name="cadastro/servidor/escala"><jsp:useBean
...
/><jsp:setProperty
  name="wipage" property="title" value="Processo de escalação dos eventos"
...
<html>	
	<head>
        ...
        <script type="text/javascript" src="/<wi:out var="wi.proj.id"/>/libs/js/jquery/ui/jquery.ui.autocomplete.js"></script>
		...
		<script type="text/javascript">			
			...
			$.mask.masks = $.extend($.mask.masks,{
              ...
              horaBR:{ mask: '99:99999', type:'reverse'},
                ...
                telefone0800:{ mask: '9999 999 99999'}
			});
  ...
  <SCRIPT TYPE="text/javascript" SRC="/<wi:out var="wi.proj.id"/>/libs/js/jquery/autocomplete/jquery.select-to-autocomplete.js"></SCRIPT>
  <META HTTP-EQUIV="pragma" CONTENT="no-cache" />
...
<SCRIPT TYPE="text/javascript">
function init(){
var el = document.getElementsByTagName('SELECT')['tmp.edt.seq_janela'];
comboSelect(el,'<wi:out var="tmp.edt.seq_janela"/>');
}
</SCRIPT>
...
<FORM NAME="tmp.edt" ACTION="/<wi:out var="wi.proj.id"/>/<wi:out var="wi.page.id"/>.wsp" METHOD="post" CLASS="frm"   id="formulario"><INPUT NAME="wi.page.prev" ID="wi.page.prev" VALUE="<wi:out var="wi.page.id"/>" TYPE="hidden" /><INPUT NAME="wi.token" ID="wi.token" VALUE="<wi:out var="wi.token"/>" TYPE="hidden" /><INPUT NAME="tmp.action" ID="tmp.action" VALUE="save" TYPE="hidden" /><INPUT NAME="tmp.id" ID="tmp.id" VALUE="<wi:out var="tmp.id"/>" TYPE="hidden" /><INPUT NAME="tmp.seq_servidor" ID="tmp.seq_servidor" VALUE="<wi:out var="tmp.seq_servidor"/>" TYPE="hidden" />
<div>
<wi:function expr="if(|tmp.tip_escala_evento|==D,<span style=\"color:red;\"><strong>Desastre e Alta</strong></span>,<span style=\"color:yellow;\"><strong>Média e Atenção</strong></span>)"/>
  <br />
   <br />
</div>
...
<div class="ui-widget" style="width:320px;">	
	<select name="tmp.edt.seq_responsavel" id="seq_responsavel" class="field" style="width:320px;">
		...
		<w:generateCombo name="cad_responsavel"/>
	</select>
</div>

<table cellpadding="0" cellspacing="0"><tr>
<td class="label">Janela de Acionamento:</td>
</tr><tr>
<td><SELECT NAME="tmp.edt.seq_janela" ID="tmp.edt.seq_janela" CLASS="field required">
  <OPTION VALUE="">--Selecione--</OPTION>
  <w:generateCombo name="aux_janela"/>
</SELECT></td></tr></table>
<INPUT NAME="tmp.tip_escala_evento" ID="tmp.tip_escala_evento" VALUE="<wi:out var="tmp.tip_escala_evento"/>" TYPE="hidden" />
<table cellpadding="0" cellspacing="0"><tr>
<td class="label">Observações:</td>
</tr><tr>
<td><TEXTAREA NAME="tmp.edt.txt_escala_observacoes" ID="tmp.edt.txt_escala_observacoes" COLS="60" ROWS="5" CLASS="field"><wi:out var="tmp.edt.txt_escala_observacoes"/></TEXTAREA></td></tr></table>
...
<table align="center" width="100%">
  <tr>
    <td align="center">      
      <!--button type="button" name="tmp.btn_add_novo" id="btn_add_novo" role="button" class="botoes ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" title="Clique no botão para novo cadastro" alt="Clique no botão para novo cadastro" onclick="wiOpen('/<wi:out var="wi.proj.id"/>/cadastro/chamado/cadastro.wsp');">
        <span class="ui-button-text">Novo chamado</span>
      </button-->
      <button type="button" name="tmp.btn_voltar" id="btn_voltar" role="button" class="botoes ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" title="Clique no botão para novo cadastro" alt="Clique no botão para voltar" onclick="wiOpen('/<wi:out var="wi.proj.id"/>/cadastro/servidor/cadastro.wsp?tmp.id=<wi:out var="tmp.seq_servidor"/>');">
        <span class="ui-button-text">Voltar</span>
      </button>
      <button type="submit" name="tmp.btn_salvar" id="btn_salvar" role="button" class="botoes ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" title="Clique no botão para salvar" alt="Clique no botão para salvar">
